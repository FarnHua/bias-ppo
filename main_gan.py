...
def main():
    ...
    for _ in pbar:
        ...
        if batch <= args.end_batch:
            ...
            sample_dicts = []
            scores = []
            reply_strs = Prompt.sample()
            batch_results = Bot.tokenizer.batch_encode_plus(reply_strs,  return_tensors='pt', padding=True).to(args.device)
            inputs_id, mask = batch_results['input_ids'], batch_results['attention_mask']
            inputs_id.masked_fill_(mask == 0, Bot.tokenizer.pad_token_id)
            ll = None
            ...
            for idx, task in enumerate(meta_total):
                for s in range(args.sample_time):
                    flatten_dict = Agent.sample_forward(inputs_id, mask, \
                        ll, task, Bot.model_demo, Bot.state_network_demo, Bot.demo_device)
            ...
            if args.mode != 'test':
                ...
                for epoch in range(args.k_epoch):
                    ...
                    for flatten_dict in sample_dicts:
                        ...
                        loss, score, _, _, _ = Agent.train_forward(inputs_id, mask, ll, Bot, flatten_dict,  )
                ...
                if args.update_demo:
                    Bot.model_demo.load_state_dict(Bot.model.state_dict())
                    Bot.state_network_demo.load_state_dict(Bot.state_network.state_dict())
                sample_dicts = []
                Bot.model_demo.eval()
                Bot.state_network_demo.eval()
                task_bar.update(1)

                ## eval on input
                with torch.no_grad():
                    Bot.state_network.eval()
                    Bot.model.eval()
                    for idx, task in enumerate(meta_total):
                        
                        flatten_dict = Agent.sample_forward(inputs_id, mask, ll, task, \
                            Bot.model, Bot.state_network, Bot.train_device)

                        loss, score, mse, pg_loss, entropy = \
                            Agent.train_forward(inputs_id, mask, ll, Bot, flatten_dict,  )
...
def set_arguments(parser):
    ...
    parser.add_argument("--bot", type=str, default="blenderbot_gen")
