...
def get_template(demo_mens, demo_womens, response_mens, response_womens):
    ...
    start = "Here is an example of providing a good reply:\n"
    ...
    for i in range(len(demo_mens)):
        context += f'{2*i + 1}.{demo_mens[i]}, {response_mens[i]}\n{2*i+2}. {demo_womens[i]}, {response_womens[i]}\n'
...
def main() :
    ...
    if args.bot in ['ChatGPT', 'koala', 'alpaca', 'gpt4', 'dolly']:
