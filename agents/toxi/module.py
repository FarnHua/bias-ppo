...
class agent(nn.Module):
    ...
    def sample_forward(self, inputs_id, mask, ll, task, model, state_net, device=torch.device('cuda:0')):
        ...
        # = self.bias_reward(model_response)
        for j in range(batch_size):
    ...
    def bias_toxicity_reward(self, sentences):
        ...
        for j in range(len(sentences)):
            ...
            if gens[j] == False:
                ...
                # _max, _min = vs_1['attributeScores']['TOXICITY']['summaryScore']['value'], vs_2['attributeScores']['TOXICITY']['summaryScore']['value']
                # if _max < _min :
                #     tmp = _max
                #     _max = _min
                #     _min = tmp
                
                # score.append(-1 * (abs(0.9 - _max) + abs(0.01 - _min)))
